 ...
 */
export const S_MCP_001 = {
    id: "S-MCP-001",
    name: "MCP catalog tool missing granularity declaration",
    category: "mcp-granularity",
    severity: "error",
    description: "MCP plugin catalog tool entry missing `granularity:` block (DD-333). " +
    ...
    check(entry) {
        ...
        const affected = new Set(entry.non_conformance_rationale?.affected_tools ?? []);
        const findings = [];
        for (const tool of entry.tools) {
            if (tool.granularity)
                continue;
            if (affected.has(tool.name))
                continue;
            findings.push({
                rule_id: this.id,
                ...
                name: this.name,
                message: `Tool '${tool.name}' (in plugin '${entry.name}') missing ` +
 ...
 */
export const S_MCP_002 = {
    id: "S-MCP-002",
    name: "Non-conformance rationale block malformed",
    category: "mcp-granularity",
    severity: "error",
    description: "DD-333 F.1 — non_conformance_rationale block present but malformed. " +
    ...
    check(entry) {
        ...
        if (!rationale)
            return [];
        const findings = [];
        const docPointer = "stallari-pack-spec/docs/non-conformance-rationale.md";
        ...
        if (rationale.scope_filtering_off !== true) {
            findings.push({
                ...
                category: this.category,
                name: this.name,
                message: `non_conformance_rationale.scope_filtering_off MUST be true ` +
                    `when the block is present; got ${JSON.stringify(rationale.scope_filtering_off)}. See ${docPointer}.`,
                path: `${entry.name}.non_conformance_rationale.scope_filtering_off`,
            });
        }
        // contamination_risks must be a non-empty array.
        if (!Array.isArray(rationale.contamination_risks) ||
            ...
            findings.push({
                ...
                category: this.category,
                name: this.name,
                message: `non_conformance_rationale.contamination_risks MUST be a ` +
        ...
        if (typeof rationale.reason !== "string" ||
            ...
            findings.push({
                ...
                category: this.category,
                name: this.name,
                message: `non_conformance_rationale.reason MUST be a non-empty string. ` +
                    `See ${docPointer}.`,
        ...
        if (!Array.isArray(rationale.affected_tools)) {
            findings.push({
                ...
                category: this.category,
                name: this.name,
                message: `non_conformance_rationale.affected_tools MUST be an array. ` +
                    `See ${docPointer}.`,
            ...
            const toolNames = new Set((entry.tools ?? []).map((t) => t.name));
            for (const ref of rationale.affected_tools) {
                ...
                findings.push({
                    ...
                    category: this.category,
                    name: this.name,
                    message: `non_conformance_rationale.affected_tools references tool ` +
...
export const CATALOG_RULES = [S_MCP_001, S_MCP_002];
