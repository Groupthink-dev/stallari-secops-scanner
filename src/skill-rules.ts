...
import type { CatalogFinding, SkillRule, SkillScanContext } from "./types.js";
...
const WRITE_VERB_PREFIX =
  /^(create|update|delete|append|send|set|write|mutate|activate|adjust|broadcast|put|post|patch|remove|add|control|reconcile|issue|cancel|refund|archive|publish)/i;
...
export function isWriteClassOp(serviceOp: string): boolean {
  const op = serviceOp.includes(".") ? serviceOp.slice(serviceOp.indexOf(".") + 1) : serviceOp;
  if (READ_OVERRIDE.test(op)) return false;
 ...
 */
const USER_READ_OP =
  /\.(read|search|query_properties|subscriptions|transactions|customers|products|invoices|profit_loss|balance_sheet|entity_state|entity_list|entity_history|area_list|energy_stats|scene_list)$/i;

function userReadsContent(serviceOps: string[]): boolean {
...
function finding(ctx: SkillScanContext, message: string, pathLeaf: string): CatalogFinding {
  return {
    rule_id: RULE_ID,
    severity: "warning",
    category: CATEGORY,
 ...
 */
export const S_SKL_001: SkillRule = {
  id: RULE_ID,
  name: "Skill manifest missing v4.5 contract field",
  category: CATEGORY,
  severity: "warning",
  description:
    "Skill manifest lacks a DD-344 v4.5 contract field its service surface requires " +
  ...
  check(ctx: SkillScanContext): CatalogFinding[] {
    ...
    for (const op of writeOps) {
      if (!(op in riskClass)) {
        findings.push(
          finding(
            ctx,
            `declares write-class op '${op}' but no 'risk_class' entry covers it — ` +
              `the DD-280 two-gate authorizer cannot gate an undeclared write`,
            `risk_class[${op}]`,
          ),
        );
      }
    ...
    const hasDomainAccess =
      ctx.manifest.domain_access !== undefined && ctx.manifest.domain_access !== null;
    if (userReadsContent(serviceOps) && !hasDomainAccess) {
      findings.push(
        finding(
          ctx,
          "reads user content but declares no 'domain_access' block — " +
            "the skill is unconstrained by the DD-278 scope ACL",
          "domain_access",
        ),
      );
    }
    ...
    const hasSealed =
      Array.isArray(ctx.manifest.sealed_credentials) &&
      ctx.manifest.sealed_credentials.length > 0;
    if (bodyNeedsOp && !hasSealed) {
      findings.push(
        finding(
          ctx,
          "body references an 'op://' credential but declares no 'sealed_credentials' " +
            "block — the secret is resolved ad-hoc rather than at dispatch time",
          "sealed_credentials",
        ),
      );
    }
    ...
    const hasTrackEmits =
      Array.isArray(ctx.manifest.track_emits) && ctx.manifest.track_emits.length > 0;
    if (bodyEmitsHandoff && !hasTrackEmits) {
      findings.push(
        finding(
          ctx,
          "emits handoff-shaped work (+/handoff/ or {{track.}}) but declares no " +
            "'track_emits' block — the action leaves no DD-245 Track audit trail",
          "track_emits",
        ),
      );
    }
...
export const SKILL_RULES: SkillRule[] = [S_SKL_001];
