...
const obfuscation = {
    ...
    severity: "high",
    description: "Prompt contains obfuscated content (base64 blocks, homoglyphs, zero-width chars)",
    patterns: [
...
const socialEngineering = {
    ...
    severity: "medium",
    description: "Prompt uses urgency, authority, or emotional manipulation tactics",
    patterns: [
        /\bURGENT\b/,
        /\bEMERGENCY\b/,
        /\bIMMEDIATE(LY)?\b/,
...
const excessiveToolUse = {
    id: "SINJ-007",
    name: "Excessive tool use",
    category: "excessive-tool-use",
    severity: "medium",
    description: "Prompt requests filesystem or network access beyond what the data block declares",
    patterns: [
        /\bread_file\b/i,
        /\bwrite_file\b/i,
...
const undeclaredCapabilities = {
    ...
    severity: "low",
    description: "Prompt references tools or services not declared in the manifest",
    patterns: [],
    structural(ctx) {
        const findings = [];
        if (!ctx.declaredServices || ctx.declaredServices.length === 0)
            return findings;
