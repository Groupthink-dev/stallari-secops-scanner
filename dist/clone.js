 ...
 */
export function buildCorpusFromPacks(packs) {
    const entries = [];
    for (const { name, yaml } of packs) {
        ...
        for (const p of prompts) {
            if (p.text.length < MIN_PROMPT_LENGTH)
                continue;
            entries.push({
                pack_name: name,
                location: p.location,
 ...
 */
export function buildThreatCorpus(entries) {
    return entries
 ...
 */
export function detectClones(prompts, corpus, pack) {
    const findings = [];
    for (const p of prompts) {
        if (p.text.length < MIN_PROMPT_LENGTH)
            continue;
 ...
 */
export function matchThreats(prompts, threats) {
    const findings = [];
    for (const p of prompts) {
        if (p.text.length < MIN_PROMPT_LENGTH)
            continue;
