 ...
 */
export function scanPackSkills(packName, packYamlContent, skillSources) {
    ...
    for (const [slug, md] of skillSources) {
        if (imports.has(slug))
            continue;
        const { manifest, body } = parseSkillManifest(md);
        findings.push(...scanSkill({ skillName: slug, packName, manifest, body }));
        scanned++;
    }
